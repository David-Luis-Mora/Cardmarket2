...
export const useCartStore = defineStore("cart", {
  ...
  actions: {
    ...
    async addProduct(product: CartProduct) {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/cart/add/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Token ${token}`,
    ...
    async updateQuantity(itemId: number, quantity: number) {
      const token = localStorage.getItem("token");
      if (!token) throw new Error("No autenticado");
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/cart/${itemId}/`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Token ${token}`,
    ...
    async removeProduct(product: Product) {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/cart/delete/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Token ${token}`,
    ...
    async clearCart() {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/cart/delete/all/`, {
        method: "POST",
        headers: {
          Authorization: `Token ${token}`,
          "Content-Type": "application/json",
    ...
    async payWithWallet() {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/cart/buy-wallet`, {
        method: "POST",
        headers: {
          Authorization: `Token ${token}`,
          "Content-Type": "application/json",
